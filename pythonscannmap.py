...
# @Last modified time: 2016-08-21T23:29:14-04:00
...
parser.add_argument("--hosts", help="Indicate hosts to scan" +
                    " example: '192.168.0.10' or '192.168.1.10 192.168.1.20'")
...
def main(hosts, ports, arguments):
    ...
    print("\n"+"#"*60+"\nScan Report started on {0}"
          .format(startdate) + " at {0}".format(starttime) + "\n" + "#" * 60)
    for host in hosts:
        nm = nmap.PortScanner()
        try:
                    ...
                    for sp in scannedport:
                        if arguments == '-sV':
                            ...
                            print("Port: {0}  State: {1}".format(int(sp),
                                  ...
                                  "\tName/Product: {0}/{1}"
                                  .format(name, product) +
                                  " Version/Extrainfo: {0}/{1}"
                                  .format(version, extrainfo))
                        else:
                            print("Port: {0}  State: {1} Name: {2}"
                                  .format(int(sp),
                                          nm[host][proto][sp]['state'],
    ...
    print("\n"+"#"*60+"\nScan Report finished on {0}"
          .format(enddate) + " at {0}".format(endtime) + "\n" + "#"*60)
